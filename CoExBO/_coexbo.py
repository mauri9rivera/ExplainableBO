...
from ._dueling_acquisition_function import DuelingAcquisitionFunction, PiBODuelingAcquisitionFunction, AlphaPiBODuelingAcquisitionFunction, BaselineDuelingAcquisitionFunction, CoTrustDuelingAcquisitionFunction
...
class StateManager(TensorManager):
    def __init__(self, n_dims, beta_init=0.2, gamma_init=0.01, probabilistic_pi="pi"):
        """
    ...
    def __call__(self, t, trustbo = None):
        """
        ...
        if self.probabilistic_pi == "pi":
            gamma = self.gamma_init * (t**2)
        elif self.probabilistic_pi == "alphapi":
            gamma = self.gamma_init
        elif self.probabilistic_pi == 'trustCo':
            gamma = self.handle_alpha(trustbo)
        else:
            gamma = self.gamma_init / (t+1)
        print(f"{t}) parameters: beta {beta:.3e} gamma {gamma:.3e}")
        return beta, gamma
    
    def handle_alpha(self, trustbo):

        return trustbo.alpha



class AlphaPIBOwithSimulation:
    def __init__(
        self, 
        domain, 
        true_function, 
        ...
        - raw_samples: int, numner of initial samples for acquisition function optimization for querying.
        - acqf_method: string, the acquisiton function. select from ["ts", "nonmyopic", "dueling"]. "dueling" is CoExBO AF.
        - uncertain_prior: bool, whether or not we use the uncertainty in prior estimation.
        - hallucinate: bool, whether or not we condition the GP on the normal BO query point.
        - adversarial: bool, (for test). If true, our selection will be reversed.
        """
        self.domain = domain
    ...
    def generate_pairwise_candidates(self, model, beta, prior_pref=None, gamma=None):
        ...
        if self.learn_preference:
            # CoExBO acquisition function is to learn human preference.
            if self.uncertain_prior:
                # (Default) Includes the uncertainty on expert preference elicitation
                acqf = AlphaPiBODuelingAcquisitionFunction(
                    model, 
                    prior_pref, 
                    beta, 
                    gamma,
...
class CoTrustBOwithSimulation:
    ...
    def generate_pairwise_candidates(self, model, beta, prior_pref=None, gamma=None):
        ...
        acqf = CoTrustDuelingAcquisitionFunction(
            model, 
            prior_pref, 
            beta, 
            self.alpha,
    ...
    def __call__(self, dataset_obj, dataset_duel, beta, gamma, sigma=None):
        ...
        self.update_history(torch.cat((X, X_next), dim=0), torch.cat((Y, Y_next), dim=0), X_pairwise_next, model, prior_pref)
        
        dataset_obj, dataset_duel, result = self.update_and_evaluate(result, dataset_obj, dataset_duel, dataset_obj_new, dataset_duel_new)
        return result, dataset_obj, dataset_duel
    
    def update_history(self, X, Y, X_pairwise_next, model, pref_model):
    ...
    def update_and_evaluate(self, result, dataset_obj, dataset_duel, dataset_obj_new, dataset_duel_new):
        ...
        #print(f'This is datset_obj_new: {dataset_obj_new}')
        #print(f'The dataset_duel_new: {dataset_duel_new}')
        ...
        correct_answer_rate = self.duel.evaluate_correct_answer_rate(X_pairwise, y_pairwise)
        temp = self.alpha
        self.alpha = self.duel.update_trust(dataset_obj, self.history)
        results = [overhead, best_obs, dist, correct_answer_rate]
        return dataset_obj, dataset_duel, results
